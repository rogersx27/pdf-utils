...
Procesa extractos bancarios de cuentas de ahorro y tarjetas de crédito
...
from pdf_analyzer.concerns import PasswordAwareMixin
from pdf_analyzer.models import Transaction, AccountSummary
from pdf_analyzer.services.parsers.number_parser import NumberParser


class BancolombiaExtractor(PasswordAwareMixin):
    """Extractor especializado para extractos de Bancolombia."""
    ...
    def _parse_transaction_row(
        ...
        description = (
            row[col_indices.get("description", 1)] if "description" in col_indices else ""
        )
        ...
        # Campos opcionales
        authorization = (
            row[col_indices.get("authorization", -1)]
            if "authorization" in col_indices
            else None
        )
        installments = (
            row[col_indices.get("installments", -1)]
            if "installments" in col_indices
            else None
        )
    ...
    def extract_to_csv(self, pdf_path: str, output_path: str) -> None:
